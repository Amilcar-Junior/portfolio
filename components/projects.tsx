...
export function Projects() {
  ...
  const ProjectCard = ({ project, index }: { project: typeof projects[0], index: number }) => (
    ...
    >
      <Card
        className={`relative overflow-hidden cursor-pointer ${isDark ? 'bg-gray-800' : 'bg-white'}`}
        ...
        <CardContent className="p-0">
          ...
          {hoveredIndex === index && (
            <motion.div
              ...
              className={`absolute inset-0 ${isDark ? 'bg-black/80' : 'bg-white/80'} p-6 flex flex-col justify-center items-center ${isDark ? 'text-white' : 'text-gray-900'}`}
            >
  ...
  return (
    <section className={`h-screen ${isDark ? 'bg-black' : 'bg-white'} py-24 flex flex-col justify-center`} id="projects">
      <SectionWrapper>
